...
import send.org.sendwallet.ui.contacts_activity.ContactsActivity;
...
public class BaseDrawerActivity extends SendActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = sendApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = sendModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (sendModule.getChainHeight()+10>peerHeight) {
            ...
            if (sendModule.isAnyPeerConnected()) {
                long peerHeight = sendModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (sendModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    private double calculateBlockchainSyncProgress() {
        long nodeHeight = sendModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (sendModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
