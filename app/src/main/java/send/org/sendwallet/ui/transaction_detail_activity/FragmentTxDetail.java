...
import send.org.sendwallet.ui.base.tools.adapter.BaseRecyclerViewHolder;
import send.org.sendwallet.ui.base.tools.adapter.ListItemListeners;
import send.org.sendwallet.ui.wallet_activity.TransactionWrapper;
import send.org.sendwallet.utils.DialogsUtil;
import wallet.exceptions.TxNotFoundException;

import static send.org.sendwallet.ui.transaction_send_activity.custom.inputs.InputsActivity.INTENT_NO_TOTAL_AMOUNT;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (intent!=null){
            ...
            if (intent.hasExtra(IS_DETAIL)){
                transactionWrapper.setTransaction(sendModule.getTx(transactionWrapper.getTxId()));
                isTxDetail = true;
            }else {
                transactionWrapper.setTransaction(new Transaction(sendModule.getConf().getNetworkParams(),intent.getByteArrayExtra(TX)));
        ...
        if (transactionWrapper.isStake()){
            ...
            try {
                ...
                for (TransactionInput input : transactionWrapper.getTransaction().getInputs()) {
                    ...
                    inputsSum = inputsSum.plus(sendModule.getUnspentValue(unspent.getHash(), (int) unspent.getIndex()));
                }
            ...
            if (transactionWrapper.getOutputLabels()!=null && transactionWrapper.getOutputLabels().containsKey(transactionOutput.getIndex())){
                ...
                if (addressLabel !=null) {
                    if (addressLabel.getName() != null) {
                        ...
                        label = transactionOutput.getScriptPubKey().getToAddress(sendModule.getConf().getNetworkParams(),true).toBase58();
                }else {
                    label = transactionOutput.getScriptPubKey().getToAddress(sendModule.getConf().getNetworkParams(),true).toBase58();
                }
            }else {
                ...
                if (script.isPayToScriptHash() || script.isSentToRawPubKey() || script.isSentToAddress()) {
                    label = script.getToAddress(sendModule.getConf().getNetworkParams(), true).toBase58();
                }else {
    ...
    private void setupOutputs(List<OutputUtil> list) {
        ...
        ListItemListeners<OutputUtil> listItemListener = new ListItemListeners<OutputUtil>() {
            ...
            public void onLongItemClickListener(OutputUtil data, int position) {
                if (sendModule.chechAddress(data.getLabel())) {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.txt_inputs){
            try {
                ...
                bundle.putSerializable(INTENT_EXTRA_UNSPENT_WRAPPERS, (Serializable) sendModule.convertFrom(transactionWrapper.getTransaction().getInputs()));
