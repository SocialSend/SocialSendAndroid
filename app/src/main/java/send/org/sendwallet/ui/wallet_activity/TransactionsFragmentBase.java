...
import send.org.sendwallet.ui.base.BaseRecyclerFragment;
...
import send.org.sendwallet.ui.transaction_detail_activity.TransactionDetailActivity;

import static send.org.sendwallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = sendModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (sendRate!=null) {
                    localCurrency = sendApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * sendRate.getValue().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + sendRate.getCoin();
                ...
                holder.title.setText(getAddressOrContact(sendModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(sendModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(sendModule.getConf().getNetworkParams()).toBase58());
                }*/
