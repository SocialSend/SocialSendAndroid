...
import sendtrum.messages.BaseMsg;
import sendtrum.messages.GetBalanceMsg;
import sendtrum.messages.GetHeader;
import sendtrum.messages.GetHistoryMsg;
import sendtrum.messages.GetTxMsg;
import sendtrum.messages.ListUnspentMsg;
import sendtrum.messages.Method;
import sendtrum.messages.SubscribeAddressMsg;
...
public class SendtrumPeer implements IoHandler{

    private final Logger log;

    /** Peer data */
    private SendtrumPeerData peerData;
    /**  */
    ...
    public SendtrumPeer(SendtrumPeerData peerData,IoManager ioManager,VersionMsg versionMsg) {
        this.peerData = peerData;
        this.ioManager = ioManager;
        this.versionMsg = versionMsg;
        this.log = LoggerFactory.getLogger(SendtrumPeer.class.getName()+"-"+peerData.getHost());
    }
    ...
    public void connect() throws ConnectionFailureException, InterruptedException {
        if (isInitilizing.compareAndSet(false,true) && !isRunning.get()) {
            ...
            throw new IllegalStateException("SendtrumPeer already initializing");
        }
    }
    ...
    public SendtrumPeerData getPeerData() {
        return peerData;
    }
