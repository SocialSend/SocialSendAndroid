...
import sendtrum.exceptions.InvalidPeerVersion;
import sendtrum.listeners.AddressListener;
import sendtrum.listeners.PeerDataListener;
import sendtrum.listeners.PeerListener;
import sendtrum.messages.VersionMsg;
...
public class SendtrumPeergroup implements PeerListener, PeerDataListener {

    private static final Logger log = LoggerFactory.getLogger(SendtrumPeergroup.class);
    ...
    /** Trusted peer */
    private SendtrumPeer trustedPeer;
    // Currently active peers.
    private final CopyOnWriteArrayList<SendtrumPeer> peers;
    // Currently connecting peers.
    private final CopyOnWriteArrayList<SendtrumPeer> pendingPeers;
    ...
    public SendtrumPeergroup(NetworkConf networkConf, WalletManager walletManager, AddressStore addressStore) throws IOException {
    ...
    public SendtrumPeergroup(NetworkConf networkConf) throws IOException {
        this.peers = new CopyOnWriteArrayList<>();
        this.pendingPeers = new CopyOnWriteArrayList<>();
        this.networkConf = networkConf;
        this.ioManager = new IoManager(1,1);
    ...
    public synchronized void start() throws InterruptedException, ConnectionFailureException {
        try {
            log.info("Starting SendtrumPeergroup");
            isActive = true;
            // todo: first part discovery..
            /*
            * Connect to the trusted node and get servers from it.
            */
            trustedPeer = new SendtrumPeer(networkConf.getTrustedServer(), ioManager,versionMsg);
            ...
            isRunning = false;
            isActive = false;
            log.error("SendtrumPeerGroup start",e);
            throw e;
        }
    }
    ...
    @Override
    public void onConnected(SendtrumPeer sendtrumPeer) {
        try {
            if (sendtrumPeer == trustedPeer) {
                ...
                for (PeerListener peerConnectionListener : peerConnectionListeners) {
                    peerConnectionListener.onConnected(sendtrumPeer);
                }
                ...
                for (InetSocketAddress inetSocketAddress : networkConf.getNetworkServers()) {
                    SendtrumPeerData peerData = new SendtrumPeerData(inetSocketAddress.getHostName(),inetSocketAddress.getPort(),0);
                    SendtrumPeer peer = new SendtrumPeer(peerData,ioManager,versionMsg);
                ...
                log.info("Non trusted peer connected, "+sendtrumPeer.getPeerData());
                pendingPeers.remove(sendtrumPeer);
                peers.add(sendtrumPeer);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    @Override
    public void onDisconnected(SendtrumPeer sendtrumPeer) {

    }

    @Override
    public void onExceptionCaught(SendtrumPeer sendtrumPeer, Exception e) {
    ...
    @Override
    public void onSubscribedAddressChange(SendtrumPeer sendtrumPeer, String address, String status) {
        try {
    ...
    @Override
    public void onListUnpent(SendtrumPeer sendtrumPeer,String address, List<Unspent> unspents) {
    ...
    @Override
    public void onBalanceReceive(SendtrumPeer sendtrumPeer, String address, long confirmed, long unconfirmed) {
        try {
            if (sendtrumPeer == trustedPeer) {
    ...
    @Override
    public void onGetHistory(SendtrumPeer sendtrumPeer, StatusHistory statusHistory) {
        try {
